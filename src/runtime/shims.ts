...
/**
 * DevTools Guest-side shim
 *
 * Wraps console methods and sets up error handlers to send structured
 * debug messages to the Host via __sendEventToHost.
 *
 * Only injected when devtools is enabled in Engine options.
 */
export const DEVTOOLS_SHIM = `
(function() {
  if (typeof globalThis.__DEVTOOLS_ENABLED__ !== 'undefined') return;
  globalThis.__DEVTOOLS_ENABLED__ = true;

  var sendToHost = function(type, data) {
    if (typeof __sendEventToHost === 'function') {
      __sendEventToHost(type, data);
    }
  };
  ...
  var serializeArgs = function(args) {
    ...
    for (var i = 0; i < args.length; i++) {
      result.push(serializeArg(args[i]));
    }
    return result;
  };

  // Wrap console methods to send structured logs
  ...
  consoleLevels.forEach(function(level) {
    if (typeof console !== 'undefined' && typeof console[level] === 'function') {
      ...
      console[level] = function() {
        ...
        // Send to devtools
        sendToHost('__DEVTOOLS_CONSOLE__', {
          entry: {
            level: level,
            args: serializeArgs(arguments),
            timestamp: Date.now(),
            stack: (level === 'error' || level === 'warn') ? (new Error()).stack : undefined
          }
        });
      };
    }
