 ...
 */
export interface RillPropValueRecord extends Readonly<Record<string, RillPropValue>> {}
 ...
 */
export type RillContextProvider<T> = (props: { value: T; children?: RillChildren }) => RillChildren;
 ...
 */
export type Operation = CreateOperation | UpdateOperation | DeleteOperation | ReorderOperation;
 ...
 */
export type RegisterComponentType = (fn: RillFunctionComponent) => string | null;
 ...
 */
export type UseState = <T>(initialValue: T | (() => T)) => readonly [T, StateUpdater<T>];
...
export interface RillComponentClass {
  new <P extends RillProps, S extends ComponentState>(props: P): RillComponent<P, S>;
...
export interface RillChildrenUtils {
  map: <T>(children: RillChildren, fn: (child: RillChildren, index: number) => T) => T[];
  forEach: (children: RillChildren, fn: (child: RillChildren, index: number) => void) => void;
