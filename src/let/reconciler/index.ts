...
function isDevToolsEnabled(): boolean {
  return typeof globalThis !== 'undefined' && globalThis.__RILL_DEVTOOLS_ENABLED === true;
}

function sendDevToolsMessage(type: string, data: unknown): void {
  if (typeof globalThis !== 'undefined' && typeof globalThis.__sendEventToHost === 'function') {
...
export function createReconciler(
  ...
  const hostConfig: ExtendedHostConfig = {
    ...
    prepareForCommit(): Record<string, unknown> | null {
      // DevTools: record commit start time
      if (isDevToolsEnabled()) {
    ...
    resetAfterCommit(_container: RootContainer): void {
      ...
      // DevTools: send render timings only (tree is already on Host via operations)
      if (isDevToolsEnabled() && renderTimings.length > 0) {
        const commitDuration = commitStartTime > 0 ? performance.now() - commitStartTime : 0;

        sendDevToolsMessage('__DEVTOOLS_RENDER__', {
    ...
    // Scheduling
    scheduleTimeout: setTimeout as unknown as (
 ...
 */
// Cache for wrapped components to preserve identity
...
function transformGuestElement(element: unknown): ReactElement | null {
  ...
  if (isFragment) {
    ...
    // Wrap function components to transform their output
    const originalType = transformedType as object;
    if (componentWrappers.has(originalType)) {
      transformedType = componentWrappers.get(originalType);
    } else {
      const wrapped: React.FC<any> = (props) => {
        // Call original component
        const result = (originalType as Function)(props);
        // Transform result
        return transformGuestElement(result);
      };
      // Copy static properties (displayName, etc)
      Object.assign(wrapped, originalType);
      componentWrappers.set(originalType, wrapped);
      transformedType = wrapped;
    }
  }
  ...
  // Note: React.createElement handles key/ref extraction from config
  const config = {
    ...transformedProps,
    key: el.key,
    ref: el.ref,
  ...
  // If transformedType is a fragment symbol, use React.Fragment
  if (transformedType === REACT_FRAGMENT_TYPE) {
    // Fragment only accepts key (not ref), and key must be properly typed
    const fragmentConfig = {
      ...transformedProps,
      key: el.key as React.Key | null | undefined,
    };
    return React.createElement(React.Fragment, fragmentConfig);
  }
