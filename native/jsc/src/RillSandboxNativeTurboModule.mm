...
std::shared_ptr<TurboModule> RillSandboxNativeTurboModuleProvider(
  ...
  if (auto jsiModule = std::dynamic_pointer_cast<rill::jsc::RillSandboxNativeTurboModule>(module)) {
 ...
 * This ensures the sandbox modules are installed as soon as the runtime is ready
 */
static void installRillSandboxNative(RCTBridge *bridge) __attribute__((constructor));
static void installRillSandboxNative(RCTBridge *bridge) {
  ...
  [[NSNotificationCenter defaultCenter]
      ...
      usingBlock:^(NSNotification * _Nonnull note) {
        ...
        // The JSI bindings will be installed by the TurboModule system
      }];
}
